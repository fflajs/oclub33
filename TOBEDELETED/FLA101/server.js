...
const app = express();
const port = 3002; // Development port

const pool = new Pool({
    user: 'gauss',
    host: 'localhost',
    database: process.env.DB_DATABASE || 'gaussdb_dev',
...
pool.connect((err, client, release) => {
    ...
    client.query('SELECT NOW()', (err, result) => {
        ...
        console.log(`Database connection to "${client.database}" successful.`);
    });
});
...
app.get('/api/saved-files', async (req, res) => {
    try {
        ...
        let query;
        let queryParams = [user.org_unit_id, user.id];
        ...
        if (user.is_manager) {
            query = `
                ...
                WHERE (org_unit_id = $1) OR (org_unit_id IN (SELECT id FROM subordinate_units) AND survey_type = 'calculated')
                ORDER BY filename ASC;
            `;
        } else {
            query = `SELECT filename FROM surveys WHERE person_id = $2 ORDER BY filename ASC;`;
        }
...
app.post('/api/saved-files', async (req, res) => {
    try {
        ...
        const filename = `cognitive_data_person_${personId}.json`;

        const upsertQuery = `
            INSERT INTO surveys (filename, survey_type, survey_results, analysis_voxel, analysis_graphs, person_id, org_unit_id)
            ...
            DO UPDATE SET
                ...
                created_at = NOW();
        `;
        const values = [filename, JSON.stringify(survey_results), JSON.stringify(analysis.voxel), JSON.stringify(analysis.graphs), personId, orgUnitId];
        
        await pool.query(upsertQuery, values);
        res.status(201).json({ message: 'Survey data saved successfully', filename });
    } catch (error) { 
...
app.post('/api/calculate', async (req, res) => {
    try {
        ...
        const sourceSurveysResult = await pool.query(
            `SELECT filename, survey_results, analysis_voxel, analysis_graphs 
             FROM surveys 
             WHERE org_unit_id = $1 AND survey_type = 'individual'`, 
            [orgUnitId]
        );

        if (sourceSurveysResult.rows.length === 0) {
            return res.status(404).json({ message: "No individual surveys found in this unit to calculate." });
        }
        ...
        for (const resultSet of allSurveyResults) { for (let i = 0; i < 120; i++) { averagedSurveyResults[i] += resultSet[i] / numFiles; } }
        const getMode = (arr) => {
        ...
        const newFilename = `calc_unit_${orgUnitId}.json`;
        ...
        const upsertQuery = `
            ...
            DO UPDATE SET
                ...
                created_at = NOW();
        `;
        const insertValues = [newFilename, JSON.stringify(averagedData.survey_results), JSON.stringify(averagedData.analysis.voxel), JSON.stringify(averagedData.analysis.graphs), user.id, orgUnitId];
        await pool.query(upsertQuery, insertValues);
...
// --- API ROUTES FOR ORG CHART (STABLE VERSION) ---
...
// --- API ROUTES FOR PERSONS (STABLE VERSION) ---
